...
ЗАДАЧА:
Проанализировать показатели и вернуть строго структурированный ответ.

ПРАВИЛА:
1. Используй только данные из входного JSON. Ничего не придумывай.
2. Для каждого показателя:
   - Нормализуй название (Конкретные общепринятое медицинское название на русском языке, Названия показателей должны быть пригодны для использования как ключи в базе данных.).
   - Укажи точное значение с единицей измерения.
   - Определи статус:
     "норма" / "выше нормы" / "ниже нормы"
   - Оценка должна учитывать возраст и пол.
3. Если показатель не имеет референсов — укажи статус "неизвестно".
4. Не объединяй показатели. Один показатель = одна строка.
5. Не добавляй лишние показатели.
...
Рекомендации:
- <конкретное действие, связанное с отклонениями>
- <ещё рекомендация>
...
- Рекомендации только по отклонениям (если всё в норме — написать "отклонений не выявлено")
"""
