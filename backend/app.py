...
INDICATOR_ALIASES = {
    ...
    "глюкоз": "Глюкоза",
    "glucose": "Глюкоза",
    # Холестерин — порядок важен: сначала специфичные, потом общий
    "холестерин лпнп": "ЛПНП (плохой холестерин)",
    "холестерин лпвп": "ЛПВП (хороший холестерин)",
    "холестерин не-лпвп": "Холестерин не-ЛПВП",
    "холестерин общ": "Холестерин общий",
    ...
    "билирубин непрям": "Билирубин непрямой",
    "direct bilirubin": "Билирубин прямой",
    "indirect bilirubin": "Билирубин непрямой",
    ...
    "urea": "Мочевина",
    # Мочевая кислота
    "мочевая кислот": "Мочевая кислота",
    "uric acid": "Мочевая кислота",
    # Общий белок
    "общий белок": "Общий белок",
    "total protein": "Общий белок",
    "белок общ": "Общий белок",
    # Альбумин
    "альбумин": "Альбумин",
    "albumin": "Альбумин",
    # Щелочная фосфатаза
    "щелочная фосфатаз": "Щелочная фосфатаза",
    ...
    "фолиев": "Фолиевая кислота",
    "folate": "Фолиевая кислота",
    "folic": "Фолиевая кислота",
    "витамин b9": "Фолиевая кислота",
    # Железо
    "железо сывороточ": "Железо",
    "сывороточное железо": "Железо",
    "железо": "Железо",
    "iron": "Железо",
    "fe": "Железо",
    # Ферритин
    ...
    "gfr": "СКФ (скорость клубочковой фильтрации)",
}
...
def normalize_indicator_name(raw_name: str) -> str:
    ...
    # Короткие алиасы (≤3 символа латиницы/цифр) проверяем по границе слова,
    # чтобы "fe" не срабатывало внутри "ferritin" и т.п.
    SHORT_KEYS = {"hb", "tg", "fe", "ca", "lh"}

    # Сортируем по убыванию длины ключа — более специфичные совпадают первыми
    for key, canonical in sorted(INDICATOR_ALIASES.items(), key=lambda x: -len(x[0])):
        if key in SHORT_KEYS:
            pattern = r'\b' + re.escape(key) + r'\b'
            if re.search(pattern, lower_clean) or re.search(pattern, lower):
                return canonical
        else:
            if key in lower_clean or key in lower:
                return canonical

    # Если не нашли в словаре — чистим форматирование
